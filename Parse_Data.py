...
def load_numpy_data(
    save_folder: Path, trigger: bool = True
) -> tuple[np.ndarray, ...]:
    """
    ...
    Returns:
        i_diff (2D np.ndarray): all currents measured by the Differential circuit. Rows correspond to time, while columns correspond to voltage.
        i_opa (2D np.ndarray): all currents measured by the Op-Amp circuit. Rows correspond to time, while columns correspond to voltage.
        v_diff (2D np.ndarray): the y-axis bias voltage values, each row is the corresponding voltage of a row in `i_diff`.
        v_opa (2D np.ndarray): the y-axis bias voltage values, each row is the corresponding voltage of a row in `i_opa`.
        times (1D np.ndarray): the x-axis time values, each entry is the timestamp of one row in `i_diff` and `i_opa.
        trigger_data (2D np.ndarray): trigger signals for the data. Only returned if `trigger` is true.
